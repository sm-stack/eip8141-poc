...
async function main() {
  ...
  if (frame0Status !== "0x1") {
    throw new Error(`Frame 0 (VERIFY/sender): got ${frame0Status}, want 0x1`);
  }
  success("Frame 0: VERIFY/sender SUCCESS (0x1)");

  // Frame 1: paymaster VERIFY succeeds.
  const frame1Status = receipt.frameReceipts[1].status;
  if (frame1Status !== "0x1") {
    throw new Error(`Frame 1 (VERIFY/paymaster): got ${frame1Status}, want 0x1`);
  }
  success("Frame 1: VERIFY/paymaster SUCCESS (0x1)");
