...
library StrictWebAuthn {
    ...
    function verify(
        ...
        if (a.authenticatorData.length != 37 || a.clientDataJSON.length > 512) return false;
        if (bytes32(a.authenticatorData[:32]) != rpIdHash) return false;
        uint8 flags = uint8(a.authenticatorData[32]);
        if (flags & 5 != 5 || flags & 0xe2 != 0 || (flags & 0x10 != 0 && flags & 8 == 0)) return false;
        bytes memory prefix = abi.encodePacked(
            '{"type":"webauthn.get","challenge":"', Base64.encode(abi.encodePacked(challenge), true, true)
        );
        if (a.clientDataJSON.length <= prefix.length) return false;
        if (keccak256(a.clientDataJSON[:prefix.length]) != keccak256(prefix)) return false;
        bytes32 suffix = keccak256(a.clientDataJSON[prefix.length:]);
        if (suffix != suffixHash && suffix != legacySuffixHash) return false;
        bytes32 digest = sha256(abi.encodePacked(a.authenticatorData, sha256(a.clientDataJSON)));
        return verifyP256(digest, a.r, a.s, x, y);
    }

    // No software fallback or code-identity read before the precompile. Both
    // high-S and low-S signatures are accepted, as specified by EIP-7951.
